<template>
  <div v-if="loginPageVisibility" class="content"
    ...
    <div v-if="loginFormVisibility" class="form">
      <div className="logo-container"><img src="../public/logo.png" alt="logo"></div>
      <input type="email" v-model.trim="userEmail" v-bind:class="{ empty_input: userEmailIsEmpty }" placeholder="Email">
      <input class="last-input" type="password" v-model.trim="userPass" v-bind:class="{ empty_input: userPassIsEmpty }"
        placeholder="Пароль">
    ...
    <div className="wrapper">
      <div class="sidenav" v-bind:class="{ sidenav_hidden: sidenavIsHidden }">
        ...
        <div className="sidenav__top-block">
          <img src="../public/img/logo-color.png" alt="logo">
        </div>
        <nav className="sidenav__block">
          ...
          <button v-if="listUsersBtnVisibility" id="sidenav__userlist" @click="openUserListPage()"
            ...
            <div class="icon-personal-area" v-bind:class="{ icon_personal_area_active: pages.listUsersPageVisibility }">
            </div>
            Список пользователей
          </button>
          <div class="separator"></div>
          <button id="sidenav__map" @click="openMap()" v-bind:class="{ sidebtn_active: pages.mapPageVisibility }">
      ...
      <div id="page-content" class="page-content" v-bind:class="{ content_compressed: contentIsCompressed }">
        <nav class="top-menu" v-bind:class="{ top_menu_compressed: contentIsCompressed }">
          <div class="top-menu__item" @click="openExtraMenu()"
            ...
            <div class="top-menu__item-name">
              <p class="menu__user-name" v-bind:class="{ personalarea_active: pages.personalAreaPageVisibility }"> {{
                saveUserData.first_name + ' ' + saveUserData.last_name }}</p>
              <p class="menu__user-role" v-bind:class="{ personalarea_active: pages.personalAreaPageVisibility }">
            ...
            <nav class="top-menu__item-options" v-bind:class="{ display_flex: extraMenuIsHidden }">
              <button id="button-pa" @click="openPersonalArea()"><i className="icon-personal-area"></i>Личный
                кабинет</button>
        ...
        <TheListPage v-if="pages.listPageVisibility" @openMainControllerPage="openMainControllerPage" :access="access"
...
export default {
  ...
  methods: {
    ...
    createProjectPost() {
      axios.post('http://e-mon.io-tech.ru/api/accounts/signup/',
        {
          "name": this.projName,
    ...
    logIn() {
      ...
      if (!this.userEmailIsEmpty && !this.userPassIsEmpty) {
        ...
        axios.post('http://e-mon.io-tech.ru/api/auth/token/login/', {
    ...
    drawLogIn() {
      ...
      axios.get('http://e-mon.io-tech.ru/api/users/me/',
        {
...
.form {
  ...
  box-shadow: 0 0 2px 0 rgba(55, 53, 53, 0.6);
  border-radius: 4px;
}
