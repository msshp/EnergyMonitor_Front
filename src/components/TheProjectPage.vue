...
export default {
    ...
    methods: {
        sendNewProj() {
            ...
            axios.patch(`http://e-mon.io-tech.ru/api/accounts/${this.projectInfo.id}/`,
                {
                    "name": this.projName,
    ...
    mounted() {
        ...
        axios.get(`http://e-mon.io-tech.ru/api/accounts/me/`,
            {
        ...
        axios.get(`http://e-mon.io-tech.ru/api/devices/limited/`,
            {
                headers: { 'Authorization': `Token ${sessionStorage.getItem('token')}` }
            }).then((response) => {
                // заполнение полей
