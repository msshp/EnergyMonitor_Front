...
export default {
    ...
    methods: {
        ...
        sendNewController() {
            axios.post('http://e-mon.io-tech.ru/api/devices/',
                {
    ...
    mounted() {
        axios.get('http://e-mon.io-tech.ru/api/device_types/', // Список типов устройств
            {
        ...
        axios.get('http://e-mon.io-tech.ru/api/installers/', // ID аккаунта сервисной компании
            {
                headers: { 'Authorization': `Token ${sessionStorage.getItem('token')}` }
            }).then((response) => {
                // обработка успешного запроса
