...
export default {
    ...
    methods: {
        ...
        getControllerData() {
            ...
            axios.get(`http://e-mon.io-tech.ru/api/devices/${this.controllerId}/fixdata/?date_start=${this.dateStart}&limit=100000`,
                {
        ...
        getErrors() {
            axios.get(`http://e-mon.io-tech.ru/api/devices/${this.controllerId}/event/?date_start=${this.dateStart}`, // вкладка «События»
                {
                    headers: { 'Authorization': `Token ${sessionStorage.getItem('token')}` }
                }).then((response) => {
                    if (response.status === 200) {
        ...
        getEventsForDashbard() {
            ...
            axios.get(`http://e-mon.io-tech.ru/api/devices/${this.controllerId}/event/?date_start=${formattedDate}`,
                {
                    headers: { 'Authorization': `Token ${sessionStorage.getItem('token')}` }
                }).then((response) => {
                    if (response.status === 200) {
        ...
        searchLastEntry() {
            axios.get(`http://e-mon.io-tech.ru/api/devices/${this.controllerId}/fixdata/?date_start=2024-05-01&limit=10000`,
                {
                    headers: { 'Authorization': `Token ${sessionStorage.getItem('token')}` }
                }).then((response) => {
                    if (response.status === 200) {
